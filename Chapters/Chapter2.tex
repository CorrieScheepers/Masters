...
In \Cref{sec:ch1_objectives} the objectives of the study are split into two main parts that need to be implemented to solve the identified problem. The two main parts are to create a logging mechanism to capture the user-based activity logs and log analysis to prioritise software maintenance as shown in \Cref{fig:ch2_developmentOfSolution}.
...
\Cref{fig:ch2_developmentOfSolution} is a high-level overview of the development of the solution. For the logging mechanism the 

\setcounter{phase}{0}
\begin{table}[!htb]
	\centering
	\caption[Development of solution design requirements]
	{\textit{Development of solution design requirements}}
	\label{tbl:ch2_developmenetRequirements}
	\begin{tabularx}{\textwidth}{|l|l|X|}
		\hline \textbf{Requirement ID} & \textbf{Name} & \textbf{Description} \\
		\hline \phase{fr:logAttributes} & Indetify log attributes & The log attributes need tobe identified what is needed to complete the log analysis for software maintenance prioritisation. The log attributes forms the characteristics of the obtained event which for this study focuses on user-based events. \\
		\hline \phase{fr:loggingPoints} & Logging point creation & The device that the user uses to access the website from where the user-based activity events are generated.\\
		\hline \phase{fr:logTools} & Create or use analysis tools & These are the captured user-based activity events that have been identified by the logging point and will be sent to the server.\\
		\hline \phase{fr:maintenancePrioritising} & Maintenance prioritising & Create a maintenance prioritising report based on the log analysis for software maintenance. \\
		\hline
	\end{tabularx}
\end{table}
...
\subsection{Logging mechanism development of solution overview}
