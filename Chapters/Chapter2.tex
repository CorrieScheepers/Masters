...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{|l|X|}
		...
		\hline F/R 1.1.1 & The event has to be triggered by the user interacting with the user interface using their device and not any other events that the system will self-initiate. The user needs to have interacted with the UI directly. \\
...
\begin{table}[!htb]
	...
	\begin{tabularx}{\textwidth}{|l|l|X|}
		...
		\hline F/R 1.4.5 & Request origin & In web applications, there are always requests sent back to the server and will call the primary function to handle the request. This can be logged as either the file that the request is being sent to or the Webpage from which the request came from. \\
...
\begin{itemize}
	\item If it alters any of the session variables,
...
\begin{itemize}
	\item The \texttt{RequestOrigin} parameter can be a file or Webpage's URL from where the user initiated the event. It can be the same as the request origin (F/R 1.4.5) of the 
\end{itemize}
