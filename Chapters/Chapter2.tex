...
\Cref{fig:ch2_database} illustrates how the connection to the database is established. The user's identity is obtained from the active session; the obtained metadata are then converted into a JSON string for storing. The final query is built and executed to store in the relational database.
...
This can by done by normalising the user-based activity data. Normalising data is a common practise in data analysis and modelling for several reasons \cite{Patro2015}:
