...
\begin{figure}[!htb] % An h :here, t: top, b: bottom.
	\centering % cent the figure
	\includegraphics[width=0.8\textwidth]{Chapter2/client_functional_requirement_flow_diagram/client_functional_requirement_flow_diagram.pdf}
...
In \Cref{fig:ch2_erdOfEventLogs} is an ERD diagram that describes the relationship of the table created to store the log attributes with other relevant tables. In the system utilisation analysis, this enables different fields of the other tables to be used to categorise the logs. In \Cref{fig:ch2_database} is flow diagram of the database interaction to store the event log data.
...
In \Cref{fig:ch2_database} the connection to the database establish. The user's identity is obtained from the active session. The obtained meta data is converted into a Json string to be able to store it. The final query is build and executed to store it into the relational database.
...
This can be achieved by implementing maintenance prioritisation using \Cref{eq:ch2_maintenanceFactorSimplified} created from the functional requirements for the maintenance prioritisation (\ref{fr:maintenanceFactor}):
...
A normalised value for the priority is used as there is not a predefined way how to describe the importance of any subsystem. In \Cref{fig:ch2_maintenancePriortisationFlow} is the log analysis flow diagram of implementing the maintenance prioritisation for a software system. \par In \Cref{fig:ch2_maintenancePriortisationFlow} the user-based activity logs are imported in log analysis tool from the relational database. The logs are aggregated into the unique subsystems. The total number of user actvities are counted aswell as the total number unique users associated with the captured logs. The maximum and minimum total user activities ($A_{Max}$ and $A_{Min}$) and users ($P_{Max}$ and $P_{Min}$) obtained for the software system. \par The normalised activity ($A_N$) for each subsystem is created from the obtained activities. The normalised priority ($P_N$) is created from the obtained maximum and minimum priority. The maintenance priority factor is created from the normalised parameters for each system and evaluted afterwards. A summary report is created for the results obtained from the log analysis using the maintenance prioritisation method.
