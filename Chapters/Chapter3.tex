...
The older subsystems of case study C use multiple logging points in their logging mechanism to capture logs. The same user-based event types as case study A's \Cref{tbl:ch3_systemAActivityTypes} are obtained through the multiple logging points.
...
\begin{itemize}
	\item \textbf{Case Study A}
		\begin{itemize}
			\item Case study A is a similar software system that is used in Case study B. Making use of a single logging point 
		\end{itemize}
	\item \textbf{Case Study B}
	\begin{itemize}
		\item Case studies A and B are similar systems that are used the same way by the users. To capture
	\end{itemize}
