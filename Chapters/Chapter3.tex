...
Case Study A is a software system \texttt{ASP.NET Core Web SDK}. The software system has 3 basic user activity types as shown in \Cref{tbl:ch3_systemAActivityTypes}.
...
Case Study B is an older system than A and C that is primarily created in \texttt{PHP}. This system has the same type of user activities as Case Study A's \Cref{tbl:ch3_systemAActivityTypes}. To capture the types of users of \Cref{tbl:ch3_systemAActivityTypes} this system used multiple logging points.\par The use of multiple logging points was to:
...
Case Study C is also a \texttt{APS.NET Core Web SDK} software system. The software system has 7 primary user activity types that are described in \Cref{tbl:ch3_systemCActivityTypes}.
...
The breakdown of user activities in \Cref{fig:ch3_caseCBreakdown} the majority of user activities for Case Study C is \texttt{CustomControl} user activity type. The more interaction with the system increases the total amount of the general user activity types (\texttt{CustomControl} and \texttt{HTMLElement}). \Cref{fig:ch3_caseCAnalysis} is the breakdown of the user activity of each subsystem's utilisation.
