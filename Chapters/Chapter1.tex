...
A defined maintenance process must be followed to improve the -accuracy and enhancements of the software. According to the \textit{IEEE Standard 1219}\footnote{\textbf{IEEE Standards} documents are developed within the Technical Committees of the Institute of Electrical and Electronics Engineers (IEEE) Societies, and the Standards Coordinating Committees of the IEEE Standards Board~\cite{Mamone1994}.}, software maintenance includes the following phases~\cite{Mamone1994, Hasan2012, Stojanov2017}:
...
\begin{itemize}
	...
	\begin{table}[!htb]
		...
		\begin{tabularx}{\linewidth}{p{3cm}X}
			...
			System resource impact & With the additional code needed to capture the logs, the usage of system resources, such as central processing unit (CPU) and I/O (input/output) channels, will increase. This may negatively impact the performance of existing system operations or increase the cost of keeping the system at the same operating speed by increasing the system resources.\\
...
These same analytics can be used for non-web-based applications because the: 
\begin{itemize}
	\item identity of the user can be captured if the software system uses a software licence and
...
\subsection{Artifical intelligence and automated logging}\label{sec:ch1_AI}
Automated logging through AI (Artificial Intelligence) offers the creation of logging points for software~\cite{Bogatinovski2023}. The primary goal is to simplify the logging process, reduce dependence on developers' expertise, and tailor logs for automated methods. While this approach brings various advantages, it also presents challenges such as concerns about performance overhead, system integrity, and quality assessment~\cite{Bogatinovski2023,Zhu2019}. \par In this study, AI-based logging approaches will not be considered over traditional logging mechanisms due to the following reasons:

\begin{itemize}
	\item \RaggedRight \textbf{Simplicity:} Traditional logging mechanisms provide a straightforward and well-defined approach to capturing user-based activity logs without the complexity of AI algorithms and models.
 
	\item \textbf{Control:} Developers retain direct control over the attributes and data captured in logs with a traditional logging mechanism, enabling specific customisation based on the application's requirements.

	\item \textbf{Resource Efficiency:} Implementing a traditional logging mechanism may require fewer computational resources than AI-based approaches, making it more suitable for specific applications or environments. Costs are also a consideration, as more specialised AI-based tools can be expensive.
	
	\item \textbf{Compatibility:} Traditional logging mechanisms enjoy widespread support and easy integration into existing software systems without needing specialised AI infrastructure.
\end{itemize}
...
\rowcolor{lightgray}
\cite{Bogatinovski2023} & \xmark & \xmark & \xmark & \cmark & \cmark & \cmark \\
