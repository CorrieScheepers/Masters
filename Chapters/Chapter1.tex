...
\subsection{Logging points and log parsing}\label{sec:Ch1_LoggignPoints}
Knowing what to log can significantly reduce any overhead the logging mechanism may produce in the software system \cite{Jia2018, Pecchia2015}. The logging quality (as described in \Cref{sec:CH1_LoggingQuality}) 
...
Any logging mechanism will need a set of defined logging points that will create the log entry \cite{Pecchia2015}. These sequence of code is responsible to identify and capture key logging information based on the current events that is happening on the system using an activation function.
