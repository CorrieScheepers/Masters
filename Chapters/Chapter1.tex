...
Automated logging using AI (Artificial Intelligence) can create logging points that can be inserted in software \cite{Bogatinovski2023}. The objective is to streamline the logging process, diminish reliance on developers' expertise, and customise logs for automated methods. Despite its benefits, implementing auto-logging poses challenges, including concerns about performance overhead, system integrity, and quality evaluation \cite{Bogatinovski2023,Zhu2019}. \par These AI-based approaches won't be considered in this study over a traditional logging mechanism due to:

\begin{itemize}
    \item Simplicity: The traditional logging mechanism provides a straightforward and well-defined approach to capturing user-based activity logs without the complexity of AI algorithms and models. 

    \item Control: With a traditional logging mechanism, developers have direct control over the attributes and data captured in the logs, allowing for specific customization based on the application's requirements.

    \item Resource Efficiency: Implementing a traditional logging mechanism may require fewer computational resources than AI-based approaches, making it more suitable for specific applications or environments. Costs are also a factor when using more specialised AI-based tools, which can be expensive.

    \item Compatibility: Traditional logging mechanisms are widely supported and easily integrated into existing software systems without needing specialised AI infrastructure.
\end{itemize}
