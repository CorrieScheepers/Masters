...
Using the defined criteria created for the state-of-the-art topics in \Cref{tbl:ch1_soaEventLogging,tbl:ch1_soaLogAnalysis,tbl:ch1_soaSoftwareMaintennace} State of the art for the obtained studies is mad in \Cref{tbl:ch1_stateOfTheArt2}.
...
The summarised comments of each study in \Cref{tbl:ch1_stateOfTheArtComments} emphasise the divide between software maintenance and event logging with the analysis of the logs included.
...
\begin{tcolorbox}[colback=gray!5!white, colframe=deepblue!80!black, title=A Systematic Review of Logging Practice in Software Engineering\cite{Rong2018}]
	...
	\hfill
	\begin{minipage}[t]{0.65\textwidth}
		This study examines how logging practices are implemented in the industry. The study also examines the number of publications on logging practices in software engineering, further discussed in \Cref{apx:loggingPractice}. This emphasizes the lack of research done for logging practices.
	\end{minipage}
...
\newcommand{\objAi}{Indentify what log attributes are needed for a comprehensive user utilisation log analysis.}
\newcommand{\objAii}{Develop a method to identify user-based events and capture the data needed to complete log attributes.}
\newcommand{\objAiii}{Investigate logging points that form the main structure of the logging mechanism to capture the user-based events and store them in a structured database.}
\newcommand{\objBi}{Evualte the log quality of the entire logging mechanism.}
\newcommand{\objBii}{Investigate how to implement maintenance prioritisation.}
\newcommand{\objBiii}{Validate the maintenance prioritisation method.}
